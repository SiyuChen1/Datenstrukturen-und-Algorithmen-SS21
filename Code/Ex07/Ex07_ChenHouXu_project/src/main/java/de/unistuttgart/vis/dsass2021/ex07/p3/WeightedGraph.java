...
public class WeightedGraph<N,E> implements IWeightedGraph<N,E> {
	...
	public IEdge<E> getEdge(int src, int dst){
		if(src > this.numNodes || src <0 || dst <0){
			throw new IllegalArgumentException();
		}
		for(int i = 0; i < this.adjacencyList.get(src).size(); i ++){
			if(dst == this.adjacencyList.get(src).get(i).getDestination()){
				return this.adjacencyList.get(src).get(i);
			}
		}
		throw new IllegalArgumentException();
	}
