package de.unistuttgart.vis.dsass2021.ex00.p3;

import static org.junit.Assert.assertEquals;

import org.junit.Assert;
import org.junit.Test;

/**
 * You should use this class to test your class {@link TextStorage}, using
 * JUnit assertions like {@link Assert#assertEquals(Object, Object)}. For
 * additional information on how to test your code with JUnit visit 
 * http://www.vogella.com/tutorials/JUnit/article.html
 */
public class TextStorageTest {

    @Test
    public void testCountMatchingCharacters(){
        final TextStorage<String> orignal = new TextStorage<>();
        final TextStorage<String> target = new TextStorage<>();
        String originalStr = "abcdefg";
        String targetStr = "abcdrf";
        orignal.set(originalStr);
        target.set(targetStr);
        assertEquals(5, orignal.countMatchingCharacters(target));

    }

}
